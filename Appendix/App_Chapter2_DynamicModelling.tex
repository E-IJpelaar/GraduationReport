...
\begin{equation}
    q_{k+1} = q_k + \gamma [J(\sigma,t)]_1^\dagger Q e_k \hspace{20pt} \text{with} \hspace{10pt}  e_k = x_d - x_k,
    \label{eq2:qupdate}
\end{equation}

where $q_k$ is modal coordinate vector at iteration $k$, $\gamma > 0$ a learning gain to enhance fast convergence $Q \in \mathbb{R}^{n\times n}$ is a diagonal matrix used to prioritize desired end-effector coordinates. In our case, the position is prioritized over the orientation. Lastly, $J^\dagger$ is an adapted form of the damped Moore–Penrose pseudo inverse defined as,
...
\While{$|| x_{set} - x_k || > \delta$}{}      \Comment{While error is larger than some $\delta$}
    ...
    \State $q_{k+1}$  $\leftarrow$ $q_{k}$ + $\alpha$ $J^{\dagger} Q e_k$ \Comment{Update generalized coordinates}
\EndWhile 
\State \textbf{end}
